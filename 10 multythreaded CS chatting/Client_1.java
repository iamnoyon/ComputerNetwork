...
public Client_1( String host )
         {         
         chatServer = host; 
  
         L = new JLabel("chatting with own pair partner");
         L.setForeground(Color.blue); 
         L1 = new JLabel("Client_1");
...
private void processConnection() throws IOException
     {
          ...
          } while ( !message.equals( "Client_2: Quit" ) );
     }  //... ... 
...
private void sendData( String message )
     {
          try // send object to server
              {
              output.writeObject( "Client_1 : " + message );
              output.flush();    
              displayMessage( "\nClient_1 : " + message );
              } 
          catch ( IOException ioException ){
...
public static void main( String args[] )
      {
         Client_1 application; 
         if ( args.length == 0 )
            application = new Client_1( "127.0.0.1" ); // connect to localhost
         else
            application = new Client_1( args[ 0 ] ); // use args to connect   
